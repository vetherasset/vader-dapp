...
export const BondItem = (props) => {
	...
	const [bondItemPrice, setBondItemPrice] = useState(ethers.BigNumber.from(0))
	...
	useEffect(() => {
		if (props.address) {
			bondPrice(props.address)
				.then((bn) => setBondItemPrice(bn))
				.catch(err => console.log(err))
		}
	}, [props.address])
	...
	return (
		<>
			{props.address	&& accountBondInfo && bondItemToken0 && bondItemPayoutToken &&
			bondItemPrice &&
					...
					>
						...
						>
							<Tag colorScheme='blue'>
								{prettifyCurrency(ethers.utils.formatUnits(bondItemPrice, 18), 0, 4)}
							</Tag>
							<Tag colorScheme='blue'>
